...
public class RestQuoteAPIVerticle extends AbstractVerticle {
	...
	public void start() throws Exception {
		vertx.eventBus().<JsonObject>consumer(GeneratorConfigVerticle.ADDRESS).handler(message -> {
			// DONE Populate the `quotes` map with the received quote
		...
		vertx.createHttpServer().requestHandler(httpRequestHandler()).listen(config().getInteger("http.port"), ar -> {
			if (ar.succeeded()) {
	...
	private Handler<HttpServerRequest> httpRequestHandler() {
		return request -> {
			...
			// Responses are returned as JSON, so don't forget the "content-type":
