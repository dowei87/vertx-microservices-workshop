...
/**
 * A verticle storing operations in a database (hsql) and providing access to
 * the operations.
 */
public class AuditVerticle extends RxMicroServiceVerticle {
	...
	/**
	 * Starts the verticle asynchronously. The the initialization is completed, it
	 * calls `complete()` on the given {@link Future} object. If something wrong
	 * happens, `fail` is called.
	 *
	...
	public void start(Future<Void> future) {
		...
		Single<HttpServer> httpEndpointReady = configureTheHTTPServer();
		Single<MessageConsumer<JsonObject>> messageConsumerReady = retrieveThePortfolioMessageSource();

		Single<MessageConsumer<JsonObject>> readySingle = Single.zip(databaseReady, httpEndpointReady,
	...
	private Single<HttpServer> configureTheHTTPServer() {
		...
		Single<HttpServer> httpServerSingle = vertx.createHttpServer().requestHandler(router::accept)
	...
	private void storeInDatabase(JsonObject operation) {
		...
		// Step 2, when the connection is retrieved (this may have failed), do the
		// insertion (upon success)
		Single<UpdateResult> update = connectionRetrieved.flatMap(
				connection -> connection.rxUpdateWithParams(INSERT_STATEMENT, new JsonArray().add(operation.encode()))
	...
	private Single<Void> initializeDatabase(boolean drop) {
		...
		// To handle such a process, we are going to create an RxJava Single and compose
		// it with the RxJava flatMap operation:
		// retrieve the connection -> drop table -> create table -> close the connection
		// For this we use `Func1<X, Single<R>>`that takes a parameter `X` and return a
		// `Single<R>` object.

		// This is the starting point of our Rx operations
		// This single will be completed when the connection with the database is
		// established.
		// We are going to use this single as a reference on the connection to close it.
		Single<SQLConnection> connectionRetrieved = jdbc.rxGetConnection();

		// Ok, now it's time to chain all these actions:
		Single<List<Integer>> resultSingle = connectionRetrieved.flatMap(conn -> {
